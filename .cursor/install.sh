...
# create python/.venv and for `python -m build`'s isolated build environments.
...
# Python package: isolate dev tooling (ruff, mypy, pytest, build, twine) in a
...
echo "Environment ready: Node workspace installed; Python venv at python/.venv"
